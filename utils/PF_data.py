...
class PFData():
    ...
    def update(
        ...
        self.pos_hips = self.__compute_pos_hips()
        self.base_pos_base_hips = self.__compute_base_pos_base_hips()
    ...
    def __compute_pos_hips(self) -> List[np.ndarray]:
        hip_frames = ['hip_L_Joint', 'hip_R_Joint']
        hip_frames_ID = [self.__pin_model.getFrameId(hip_frames[i]) for i in range(2)]
        pos_hips = []
        for hip_idx in range(2):
            pos_hipi = self.__pin_data.oMf[hip_frames_ID[hip_idx]].translation
            pos_hips.append(pos_hipi)
        return pos_hips
...
def test():
    ...
    pf_data = PFData(robot_model=robot_path)
    pf_data.update(
